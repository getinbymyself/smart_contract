...
contract MainService is Roles{
    ...
    //查询用户身份
    function getPersonroles(address _account) public view returns (string memory){
        checkEmptyAddress(_account);
        checkAccountExists(_account);
        uint8 flag = 0;
    ...
    // 房屋管理------------------
    // 添加房屋
    function addHouse(uint houseId, string memory password) public onlyLandlord{//仅有房东可添加房屋
        uint id = houseId;
        require(id != 0,"40009");
    ...
    // 修改密码
    function changeHousePasswd(uint houseId, string memory password) public onlyOwner {
        uint id = houseId;
        House storage house = houseMap[id];
